...
.controller('HomeCtrl', ['$scope', function ($scope) {
  ...
  $scope.includeCat = function(cat) {
      var i = $.inArray(cat, $scope.catIncludes);
      if (i > -1) {
          $scope.catIncludes.splice(i, 1);
      } else {
          $scope.catIncludes.push(cat);
      }
      $scope.catFilter = function(produtos) {
          if ($scope.catIncludes.length > 0) {
              if ($.inArray(produtos.cat, $scope.catIncludes) < 0)
                  return;
          }
          return produtos;
      }
  ...
  (function($){
    $(function(){
      $('.slide').slick({
        ...
        speed: 400,
        autoplay: true,
        nextArrow: $('#btn-next-principal'),
        prevArrow: $('#btn-prev-principal')
      });
    });
  })(jQuery);
